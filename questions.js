...
const uniqueKey = "Two Events Sample Space";
...
function loadQuestion(index) {
  ...
  if (randomQuestion.image) {
    ...
    questionImage.style.maxHeight = "250px";
...
function loadInputQuestion(randomQuestion, optionsElement, index) {
  ...
  // Create input fields based on the input array
  randomQuestion.input.forEach(function(inputField, idx) {
    var inputWrapper = document.createElement("div");
    inputWrapper.className = "input-wrapper";
    inputWrapper.style.display = "flex";
    inputWrapper.style.flexDirection = "column";
    inputWrapper.style.alignItems = "center";
    inputWrapper.style.margin = "5px";

    var inputElement = document.createElement("input");
    inputElement.type = "text";
    inputElement.className = "answer-input";
    inputElement.placeholder = inputField.operand || "Enter answer";
    ...
    // Add event listener to handle answer changes
  ...
  // Restore previously entered answers if they exist
  var previouslyEntered = Ansgiven[index];
  if (previouslyEntered && Array.isArray(previouslyEntered)) {
    var inputs = inputContainer.querySelectorAll('.answer-input');
...
function displayResults() {
  ...
  score = Ansgiven.reduce((total, answer, index) => {
    ...
    if (currentQuestion.input && Array.isArray(currentQuestion.input)) {
      // Handle input type questions
      if (answer && Array.isArray(answer) && Array.isArray(currentQuestion.answer)) {
        isCorrect = currentQuestion.answer.every((correctAnswer, idx) => {
          return answer[idx] && answer[idx].toLowerCase().trim() === correctAnswer.toLowerCase().trim();
        });
      }
    } else {
      // Handle regular option-based questions
  ...
  for (var page = 0; page < numberOfPages; page++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      if (quesgroup.image) {
        questionImageContent = "<br><img src='" + quesgroup.image + "' alt='Question Image' style='max-width: 300px; max-height: 200px; object-fit: contain; border-radius: 8px; margin: 10px 0;'><br>";
      }
      ...
      if (quesgroup.input && Array.isArray(quesgroup.input)) {
        ...
        if (givenAnswers && Array.isArray(givenAnswers)) {
          var isCorrect = correctAnswers.every((correctAnswer, idx) => {
            return givenAnswers[idx] && givenAnswers[idx].toLowerCase().trim() === correctAnswer.toLowerCase().trim();
          });
        ...
        var givenAnswer = Ansgiven[j] !== undefined ? questions[j].options[Ansgiven[j]] : null;
      ...
      var num = j + 1;
      pageDiv += "Q." + num + " " + ques + questionImageContent + "<br>" + "Correct Answer: " + ansContent + "<br>" + "Answer Given: " + givenContent + "<br><br>";
    }
